...
### 1. Customizar Regras de Validação do Avatar

```php
->shouldShowAvatarForm(
    value: true,
    directory: 'avatars',
    rules: 'mimes:png,jpg,jpeg|max:2048|dimensions:min:200,200|max:800,800'
)
```

### 2. Adicionar Campos Customizados

Para adicionar campos personalizados, você pode estender o plugin ou criar componentes customizados:
...
->plugin(
    FilamentEditProfilePlugin::make()
        ...
        ->shouldShowDeleteAccountForm(false)
        ->shouldShowMultiFactorAuthentication()
        ->shouldShowThemeColorForm()      // Adiciona seletor de cor do tema
        ->shouldShowLocaleForm()          // Adiciona seletor de idioma
)
```

### 3. Configurar Múltiplos Idiomas

```php
// Em config/filament-edit-profile.php
'locales' => [
    'pt_BR' => '🇧🇷 Português',
    'en' => '🇺🇸 Inglês',
    'es' => '🇪🇸 Espanhol',
...
### Problema: Avatares não são exibidos

**Causa**: Configuração incorreta do disco de armazenamento

**Solução**:
1. Verificar se o disco está configurado como `public` em `filament-edit-profile.php`
2. Confirmar que o link simbólico está criado: `php artisan storage:link`
3. Verificar permissões da pasta `storage/app/public/avatars/`
4. Verficar pelo console do navegador se a URL do avatar está sendo gerada conforme o APP_URL do .env 

### Problema: Erro de upload de arquivo

**Causa**: Regras de validação muito restritivas

**Solução**:
1. Ajustar as regras no `AdminPanelProvider`
2. Verificar se o tipo de arquivo é suportado
3. Confirmar que o tamanho está dentro do limite

### Problema: 2FA não funciona

**Causa**: Campos de banco de dados não configurados

**Solução**:
1. Verificar se as colunas existem na tabela `users`:
   - `app_authentication_secret`
   - `app_authentication_recovery_codes`
2. Executar migrations se necessário
3. Verificar se o modelo User implementa as interfaces corretas
